...
export class CursorService {
  ...
  /**
   * Generate workflow.cursorrules content from instruction templates
   */
  ...
  /**
   * Install Cursor rules files safely
   */
  async installCursorRules(
    ...
    // Check for conflicts
    const workflowPath = path.join(projectPath, 'workflow.cursorrules');
    if (await this.fileSystemService.fileExists(workflowPath)) {
      conflicts.push('workflow.cursorrules');
    }
    ...
    try {
      ...
      // Write workflow.cursorrules
      await this.fileSystemService.writeFile(workflowPath, workflowContent);

      // Write pattern-specific rules
      for (const rule of patternRules) {
        const rulePath = path.join(cursorRulesDir, rule.filename);
  ...
  async backupExistingRules(projectPath: string): Promise<string[]> {
    ...
    if (await this.fileSystemService.fileExists(workflowPath)) {
      const backupPath = await this.fileSystemService.backupFile(workflowPath);
      backups.push(backupPath);
    }

    // TODO: Add backup for .cursor/rules/*.mdc files
  ...
  /**
   * Combine multiple instruction files into workflow.cursorrules
   */
...
# Cursor AI Development Rules`;
  ...
  private createMdcContent(
    ...
    const frontmatter = `---
patterns: ["${pattern}"]
description: "AI rules for ${pattern} files"
---
